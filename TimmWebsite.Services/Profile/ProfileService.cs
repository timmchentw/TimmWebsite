...
namespace TimmWebsite.Services.Profile
{
    public class ProfileService : IProfileService
    {
        ...
        public ProfileService(IConfiguration config, IDalSession dalSession)
        {
            _config = config;
            _dalSession = dalSession;
        }

        public List<ProfilePortfolioDto> GetPortfolio()
        {
            if (false)
            {
                // Enable after database is set
                _dalSession.UseProfileTransaction(() =>
                {
                    var test = _dalSession.UnitOfWorkProfile.GenericRepo.Get<ProfilePortfolioDto>("CUSTOM SQL");
                    var entities = _dalSession.UnitOfWorkProfile.ProfilePortfolio.GetAll();
                    var resultDto = entities.Select(x => new ProfilePortfolioDto()
                    {
                        Title = x.Title,
                        Text = x.Description,
                        ImgUrl = x.ImgUrl,
                        Tags = x.Tags.Split(',').Select(y => (PortfolioTag)Enum.Parse(typeof(PortfolioTag), y)).ToList()
                    });
                });
            }


            var dtos = new List<ProfilePortfolioDto>()
            {
                new ProfilePortfolioDto()
                {
        ...
        {
            var dtos = new List<ProfileSkillDto>()
            {
                new ProfileSkillDto()
                {
        ...
        public List<ProfileTimeLineDto> GetTimeLineItems()
        {
            var dtos = new List<ProfileTimeLineDto>()
            {
                new ProfileTimeLineDto()
                {
                    Year = 2013,
                    Month = "Q4",
                    Title=  "就讀成功大學",
